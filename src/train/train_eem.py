"""Multi-target training for the combined MLIP + closed-form-EEM model.

    rsfff-train-eem configs/h2o_h3o_eem.yaml

Fits :class:`MLIPEEMModel` on energies, forces, dipoles, dipole derivatives, and
polarizabilities simultaneously, with the isolated-species integer-charge anchors
as an auxiliary loss. The EEM layer is an exact analytic minimum (Lagrange
multiplier for total charge), so every step is a single ordinary autograd graph
-- no inner solver, nothing to diverge. Each loss term is reported separately.

Notes:
- ``positions.requires_grad_`` is set before the model call (forces, d mu/dR).
- The polarizability is the model's analytic output; no applied-field tensor is
...
from ..mlip import build_eem_model
...
from .loss import eem_model_loss, isolated_species_loss
from .train import _iter_minibatches
from .train import resolve_device as _resolve_device_any


def resolve_device(name: str, dtype: str) -> torch.device:
    """Like train.resolve_device, but never auto-selects MPS for float64
    (the MPS framework has no float64 support)."""
    device = _resolve_device_any(name)
    if name == "auto" and device.type == "mps" and dtype == "float64":
...
_LOG_KEYS = (
    ...
    "alpha_mae", "iso_e_max", "q_abs_mean", "eta_min",
)
...
def main():
    parser = argparse.ArgumentParser(description="Train the combined MLIP + EEM model.")
