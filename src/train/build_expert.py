...
v4: there is no expert bank and no per-composition construction. One featurizer, one
fragment-state embedding, one decoder. Composition reaches the parameters as geometry (``h``)
and as a fractional element census inside the state block, both of which a mixture can move --
which a dictionary of networks keyed on ``"H2O"`` could not.
"""
...
from ..ff.expert import ApplicabilityHead
...
__all__ = ["build_decoder", "build_expert_model", "slot_dims"]
...
def build_decoder(featurizer, config, neighbor_types, atomic_states, dims):
    """The **one** parameter decoder every fragment shares, at feature widths.

    Built with the two-slot split in every head's first layer, exactly as the v2 per-expert
    heads were -- same classes, same priors, same positivity constraints, same
    initialization. What changed is the count: there is one of these instead of one per
    composition, so an H3O+ oxygen and an H2O oxygen are separated by their *input* rather
    than by which network ran. That is what makes a crossover between them mean anything.
    """
    ...
    response = build_response(
        ...
        p0_extra=dims.p0_frag - featurizer.feature_dims[0], slots=dims,
    )

    log_c6, log_b_disp = build_log_priors(neighbor_types, b_prior=config.dispersion.b_prior)
    disp_params = DispersionParameterHeads(
        dims.p0_frag, n_species, log_c6_prior=log_c6, log_b_prior=log_b_disp,
        p_env=dims.p0_env,
    ...
    to_spherical = (
        irrep2_to_spherical(featurizer.backend.irrep6_to_voigt()) if dims.p2_frag else None
    )
    log_q, log_b_pauli, mu_scale, quad_scale = build_pauli_priors(neighbor_types)
    pauli_params = PauliMultipoleHeads(
        dims.p0_frag, dims.p1_frag, n_species,
        log_q_prior=log_q, log_b_prior=log_b_pauli, dipole_scale=mu_scale,
        p2=dims.p2_frag, quad_scale=quad_scale, irrep2_to_spherical=to_spherical,
        p_env=dims.p0_env, p1_env=dims.p1_env, p2_env=dims.p2_env,
    ...
    # `r0` reads the element and nothing else. `environment_r0` is forced off and no descriptor
    # is passed: the whole point is that `r0` cannot move when the description of an atom
    # changes. See `rsfff.ff.decoder`, and do not let the fragment state in here either.
    ...
    bond = FragmentBondEnergy(
        dims.p0_frag, n_species, p1=dims.p1_frag, p2=dims.p2_frag,
        p_env=dims.p0_env, p1_env=dims.p1_env, p2_env=dims.p2_env,
...
def build_expert_model(config, neighbor_types, reference_energies, atomic_states=None):
    """The whole model: one featurizer, one fragment-state block, one decoder."""
    ...
    decoder = build_decoder(featurizer, config, neighbor_types, atomic_states, dims)
