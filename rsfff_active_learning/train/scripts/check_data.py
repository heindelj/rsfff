...
#: ``--force-only``: the force-only stream's files (``data.force_path``) carry no EDA
REQUIRED_FORCE_ONLY = ("energy",)
OH_MAX = 1.3
KV = re.compile(r'(\w+)=("([^"]*)"|\S+)')
...
def check_file(path: Path, seen: dict, problems: list, levels: Counter,
               allow_duplicates: bool = False, required=REQUIRED) -> tuple[int, Counter]:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--force-only", action="store_true",
                    help="files of the force-only stream: energy + forces, no EDA labels")
    ...
    for path in files:
        n, sizes = check_file(path, seen, problems, levels, args.allow_duplicates,
                              REQUIRED_FORCE_ONLY if args.force_only else REQUIRED)
        total += n
